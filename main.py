...
# GUI
root = Tk()
photo = ImageTk.PhotoImage(Image.open(r"projects\voice_assistant\1.png").resize((50, 50)))
photo1 = ImageTk.PhotoImage(Image.open(r"projects\voice_assistant\2.png").resize((50, 50)))
...
data_path = r"projects\voice_assistant\data.json"
media = None
...
def create_task(query):
    ...
    with open(r"projects\voice_assistant\todo-list.txt", "a", encoding="utf-8") as file:
        file.write(f"❗️ {task}\n")
    talk(f"Task {task} added to todo-list!")


def clear_todo_list():
    f = open(r"projects\voice_assistant\todo-list.txt", "w")
    f.close()
