...
def update_data(new_data):
    ...
    with open(data_path) as f:json.dump(new_data,f)

def is_full_screen():
    try:return win32gui.GetWindowRect(user32.GetForegroundWindow()) == (0, 0, user32.GetSystemMetrics(0), user32.GetSystemMetrics(1))
    except:return False
...
def play(words):
    ...
    try:song = ' '.join(words[words.index('play')+1:])
    except:talk('Play what?');print('Listening...');song = listen_command(False)
    video = new("https://www.youtube.com/watch?v=" + "{}".format(re.findall(r"watch\?v=(\S{11})", urllib.request.urlopen("https://www.youtube.com/results?" + urllib.parse.urlencode({"search_query": song})).read().decode())[0])) 
...
def search(words):
    try:query = ' '.join(words[words.index('search')+1:])
    except:query = ' '.join(words[words.index('google')+1:])
    talk('Searching')
    srch(query)
...
async def getweather(region):
    ...
    try:weather = await client.find(region)
    except:
        talk('Invalid region')
        getweather()
...
def change_assname(words):
    try:new_assname = words[words.index('to')+1]
    except:talk("What would you like to call me?");new_assname = listen_command(False)
